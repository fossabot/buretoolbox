...
class BuretoolboxPluginHandler():
    ...
    def __init__(self, plugin_package):
        ...
        when an instance of the BuretoolboxPluginCollection object is created
        """
    ...
    def walk_package(self, package):
        ...
        for _, pluginname, ispkg in pkgutil.iter_modules(
            ...
            if not ispkg:
                ...
                for (_, classname) in clsmembers:
                    # Only add classes that are a sub class of BuretoolboxPlugin,
                    # but NOT BuretoolboxPlugin itself
                    if issubclass(classname, BuretoolboxPlugin) & (
                            classname is not BuretoolboxPlugin):
                        print(
                            f'    Found plugin class: {classname.__module__}.{classname.__name__}'
                        )
                        self.plugins.append(classname())
